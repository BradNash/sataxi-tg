...
inputs = {
  ...
  description      = "Allows access to Postgres RDS"
  attributes       = ["postgres-rds-sg-${local.env_vars.locals.environment}"]
  ...
  rules = [
    {
      ...
      self        = null
      description = "Allow traffic on port 5432 from inside VPC"
    },
    {
      type        = "ingress"
      from_port   = 5432
      ...
      self        = null
      description = "Allow traffic on port 5432 from on Prem via VPN"
    },
    {
      type        = "ingress"
      from_port   = 5432
