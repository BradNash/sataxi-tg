...
locals {
  repository_name = "bbd-services-security-identity"
}

inputs = {
  source_repo_name                = local.repository_name
  source_repo_branch              = "master"
  source_repo_arn                 = "arn:aws:codecommit:eu-west-1:665316528893:bbd-services-security-identity"
      ...
      actions = [
        {
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              ENV                       = "dev"
              TASK_DEFINITION_FILE_PATH = "deploy/identity-service/dev/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/identity-service/dev/appsettings.json"
      ...
      actions = [
        {
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              SERVICE_NAME              = "identity",
              ENV                       = "qa"
              TASK_DEFINITION_FILE_PATH = "deploy/identity-service/qa/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/identity-service/qa/appsettings.json"
              REPOSITORY_NAME           = local.repository_name
              COMMIT_HASH               = "#{build.COMMIT_HASH}"
              IMAGE_TAG                 = "#{build.IMAGE_TAG}"
            })
      ...
      actions = [
        {
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              SERVICE_NAME              = "identity",
              ENV                       = "prod"
              TASK_DEFINITION_FILE_PATH = "deploy/identity-service/prod/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/identity-service/prod/appsettings.json"
              REPOSITORY_NAME           = local.repository_name
              COMMIT_HASH               = "#{build.COMMIT_HASH}"
              IMAGE_TAG                 = "#{build.IMAGE_TAG}"
            })
