...
locals {
  repository_name = "rewards-service"
}

inputs = {
  source_repo_name                = local.repository_name
  source_repo_branch              = "master"
  source_repo_arn                 = "arn:aws:codecommit:eu-west-1:665316528893:rewards-service"
  ...
  additional_pipelines_stages = [
    {
      ...
      actions = [
        {
          name            = "Deploy-Rewards-Service"
          namespace       = "rewards-dev"
          category        = "Invoke"
          owner           = "AWS"
          version         = "1"
          provider        = "Lambda"
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              ENV                       = "dev"
              TASK_DEFINITION_FILE_PATH = "deploy/rewards-service/dev/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/rewards-service/dev/config.yaml"
      ...
      actions = [
        {
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              SERVICE_NAME              = "rewards",
              ENV                       = "qa"
              TASK_DEFINITION_FILE_PATH = "deploy/rewards-service/qa/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/rewards-service/qa/config.yaml"
              REPOSITORY_NAME           = local.repository_name
              COMMIT_HASH               = "#{build.COMMIT_HASH}"
              IMAGE_TAG                 = "#{build.IMAGE_TAG}"
            })
      ...
      actions = [
        {
          ...
          configuration = {
            ...
            UserParameters = jsonencode({
              ...
              SERVICE_NAME              = "rewards",
              ENV                       = "prod"
              TASK_DEFINITION_FILE_PATH = "deploy/rewards-service/prod/task-definition.yaml"
              CONFIG_FILE_PATH          = "deploy/rewards-service/prod/config.yaml"
              REPOSITORY_NAME           = local.repository_name
              COMMIT_HASH               = "#{build.COMMIT_HASH}"
              IMAGE_TAG                 = "#{build.IMAGE_TAG}"
            })
